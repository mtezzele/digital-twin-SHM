...
class GetSetup:
    ...
    def _get_up_diag_transition_2step(self, prob_retr1, prob_retr2):
        ...
        for i in range(1, n_states):
            ...
            # case in which only the undamaged conditon and the first delta inteval 
            # can be recovered
    ...
    def _get_up_diag_transition_3step(self, prob_retr1, prob_retr2, prob_retr3):
        ...
        for i in range(1, n_states):
            ...
            # case in which only the undamaged conditon and the first delta interval 
            # can be recovered
            elif i-(k*self.n_level_step) == 2:
                trans[i-1, i] = prob_retr1
                trans[0, i] = prob_retr2 + prob_retr3
            # case in which only the undamaged conditon and the first and second
            # delta intervals can be recovered
...
class PhysicalAsset:
    ...
    def update_physical_state(self, action):
        ...
        :param int action: identifier of the action ruling the physical state update.
            ...
            2 --> "minor_imperfect_repair" decision.
            3 --> "major_imperfect_repair" decision.
        """
        ...
        elif action == "minor_imperfect_repair":
            # if already undamaged, restart intial condition
            if self.state[0] == 0:
                self.degradation_steps = 0
        elif action == "major_imperfect_repair":
            # if already undamaged, restart intial condition
            if self.state[0] == 0:
                self.degradation_steps = 0
        ...
        if self.degradation_steps == 0:
            ...
            if self.degradation_law == "custom":
                if action == "do_nothing" or action == "minor_imperfect_repair" or \
                action == "major_imperfect_repair":
                    if self.state[0] == 0:
                        self.state[0] = 4
                        self.state[1] = 0.31
                    else:
                ...
                if action == "do_nothing":
                    if self.state[0] == 0:
                        ...
                        if rnd.randrange(2):
                            ...
                            self.state[1] = np.random.uniform(low=0.3, high=0.7)
                    else:
                        self.state[1] += np.max((0.,np.random.normal(0.015, 0.01)))      
                elif action == "minor_imperfect_repair" and self.state[0] != 0:
                    self.state[1] -= np.random.normal(0.125,0.01)
                    if self.state[1] < 0.3:
                        self.degradation_steps = 0
                        self.state = np.array([0, 0.30])
                elif action == "major_imperfect_repair":
                    self.state[1] -= np.random.normal(0.175,0.01)
                    if self.state[1] < 0.3:
                        self.degradation_steps = 0
                        self.state = np.array([0, 0.30])
...
class DigitalAsset:
    ...
    """
    def __init__(self, classifier, regressors, statistics_level, d_level_discr, cpd=np.diag(np.ones((43)))):
    ...
    def compute_reward(self, state, action):
        ...
        def r_health(state):
            ...
            if loc_damage == 0:
                ...
                r_health = -np.exp(5*liv_damage)
            elif loc_damage > 4:
                r_health = -np.exp(4*liv_damage)
            return r_health
...
if __name__ == '__main__':
    ...
    frame = PhysicalAsset(path_data, degradation_law="stochastic")
